...
for kkk in range(1, 10):
    ...
    #     ################################################ Model ################################################
    #     # LR
    #     lr_output = tf.reduce_sum(sf_embedding)

    #     output = tf.sigmoid(lr_output)
    ...
    #     lr_output = tf.reduce_sum(sf_embedding)
    #     fm_layer = FMLayer()
    #     fm_output = fm_layer(df_embedding_split_by_field)
    #     output = tf.sigmoid(lr_output+fm_output)

    #     ################################################ Model ################################################
    #     # MLP
    #     model = DNNLayer(unit_list = [64, 32, 1],
    #                      activation_list = ['relu', 'relu', None],
    #                      bias_list = [True, True, True],
    #                      kernel_regularizer_list = [None, None, None])
    #     output = model(df_embedding)

    ################################################ Model ################################################
    # IPNN
    ip_layer = IPLayer()
    ip_embedding = tf.concat([df_embedding, ip_layer(df_embedding_split_by_field)], axis=1)
    model = DNNLayer(unit_list=[32, 1],
                     activation_list=['relu', None],
                     bias_list=[True, True],
                     kernel_regularizer_list=[None, None])
